...
const NonVeg: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const savedCart = localStorage.getItem('nonVegCart');
    if (savedCart) {
  ...
  const handleAddToCart = (item: FoodItem) => {
    ...
    localStorage.setItem('nonVegCart', JSON.stringify(updatedCart));
  ...
  const handleSignupSubmit = (e: React.FormEvent) => {
    ...
    if (cartItems.length > 0) {
      ...
      alert(`🎉 Welcome ${signupForm.name}! Your order for ${cartItems.length} non-veg items has been placed successfully!`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 to-orange-100 relative overflow-hidden">
      ...
      // src/components/NonVeg.tsx (Signup popup section matrame update chesa)
{/* Signup Popup Modal */}
{showSignup && (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
    {/* Animated Background Images - Black Background lo Top to Bottom Moving */}
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {/* Moving non-veg food images from top to bottom in black background */}
    ...
    <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-auto relative z-10">
      <div className="bg-gradient-to-r from-red-500 to-orange-600 rounded-t-2xl p-4 text-white">
      ...
      <form onSubmit={handleSignupSubmit} className="p-4 space-y-3">
        <div>
          ...
          <input
            ...
            required
            className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-200"
        ...
        <div className="flex gap-2 pt-3">
          ...
          <button
            type="submit"
            className="flex-1 bg-gradient-to-r from-red-500 to-orange-600 hover:from-red-600 hover:to-orange-700 text-white py-2 px-4 rounded-lg font-semibold text-sm transition-all duration-200 transform hover:scale-105"
          >
